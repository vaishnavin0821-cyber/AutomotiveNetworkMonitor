...
- **History-based duplicate detection** — each client tracks its last 20 processed event IDs, not just the most recent one.
...
- **Sequential broadcast** — the server processes clients one at a time rather than in parallel; a slow client can delay delivery to others.
